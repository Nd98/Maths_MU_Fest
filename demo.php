...
$temp="select id from variables where round='demo'";
$temp1=mysqli_fetch_assoc(mysqli_query($con,$temp));
$id=$temp1["id"];
mysqli_select_db($con,$db);


    $sql="select * from `demo` where id='".$id."' limit 1";
    $result=mysqli_query($con,$sql);
    $result1=mysqli_num_rows($result);
    $row=mysqli_fetch_assoc($result);
    if(is_null($row["id"])){
    	header("Location: scorecard_demo.php");
    	exit();
    }
    else{
    mysqli_select_db($con,"round_intermediate");
$id=$id+1;
    $sql1="update variables SET id='".$id."' WHERE round='demo'";
...
<html>
	<head>
		<title>Demo Questions Display</title>
...
<br>
  ...
  <div class="row">
                <div class="col-lg-18 text-center">
                    <h2 class="section-heading" style="color: #fff">DEMO</h2>
