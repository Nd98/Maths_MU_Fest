...
$temp="select id from variables where round='sub_round_1.1'";
$temp1=mysqli_fetch_assoc(mysqli_query($con,$temp));
$id=$temp1["id"];
mysqli_select_db($con,$db);


    $sql="select * from `sub_round_1.1` where id='".$id."' limit 1";
    $result=mysqli_query($con,$sql);
    $result1=mysqli_num_rows($result);
    $row=mysqli_fetch_assoc($result);
    if(is_null($row["id"])){
    	header("Location: scorecard_round_1.1.php");
    	exit();
    }
    else{
    mysqli_select_db($con,"round_intermediate");
$id=$id+1;
    $sql1="update variables SET id='".$id."' WHERE round='sub_round_1.1'";
