...
$temp="select id from variables where round='sub_round_2.3'";
$temp1=mysqli_fetch_assoc(mysqli_query($con,$temp));
$id=$temp1["id"];
mysqli_select_db($con,$db);


    $sql="select * from `sub_round_2.3` where id='".$id."' limit 1";
    $result=mysqli_query($con,$sql);
    $result1=mysqli_num_rows($result);
    $row=mysqli_fetch_assoc($result);
    if(is_null($row["id"])){
    	header("Location: scorecard_round_2.3.php");
    	exit();
    }
    else{
    mysqli_select_db($con,"round_intermediate");
$id=$id+1;
    $sql1="update variables SET id='".$id."' WHERE round='sub_round_2.3'";
...
<html>
	<head>
		<title>Round 2 Questions Display</title>
...
<br>
  ...
  <div class="row">
                <div class="col-lg-18 text-center">
                    <h2 class="section-heading" style="color: #fff">ROUND 2</h2>
